...
export const createMockQueryBuilder = <T = unknown>(
  ...
  const builder = {
    ...
    then: <TResult1 = { data: T; error: unknown; count?: number }, TResult2 = never>(
      onfulfilled?:
        | ((value: { data: T; error: unknown; count?: number }) => TResult1 | PromiseLike<TResult1>)
        | null,
      onrejected?: ((reason: unknown) => TResult2 | PromiseLike<TResult2>) | null,
    ): Promise<TResult1 | TResult2> => {
      return Promise.resolve({ data: returnData, error: returnError, count }).then(
        onfulfilled,
        onrejected,
      );
    },
...
export const createMockSupabaseClient = (
  ...
  const { userId = "test-user-id", queryData = [], queryError = null } = options;
