 ...
 * チーム一覧取得クエリ
 */
export function useTeamsQuery(supabase: SupabaseClient, options: UseTeamsQueryOptions = {}) {
  const {
    teamId,
    enableRealtime = true,
    initialTeams,
  ...
  return {
    ...
    isLoading:
      ...
      (teamId
        ? teamDetailQuery.isLoading || membersQuery.isLoading || announcementsQuery.isLoading
        : false),
    isError:
      teamsQuery.isError ||
      (teamId
        ? teamDetailQuery.isError || membersQuery.isError || announcementsQuery.isError
        : false),
    error:
      teamsQuery.error ||
      (teamId
        ? teamDetailQuery.error || membersQuery.error || announcementsQuery.error
        : undefined),
    refetch: async () => {
...
export function useCreateTeamMutation(
  ...
  api?: TeamCoreAPI,
): UseMutationResult<Team, Error, TeamInsert> {
  const queryClient = useQueryClient();
...
export function useUpdateTeamMutation(
  supabase: SupabaseClient,
  api?: TeamCoreAPI,
): UseMutationResult<Team, Error, { id: string; updates: TeamUpdate }> {
  const queryClient = useQueryClient();
  const coreApi = useMemo(() => api ?? new TeamCoreAPI(supabase), [supabase, api]);
...
export function useJoinTeamMutation(
  supabase: SupabaseClient,
  api?: TeamMembersAPI,
): UseMutationResult<TeamMembership, Error, string> {
  const queryClient = useQueryClient();
...
export function useRemoveMemberMutation(
  supabase: SupabaseClient,
  api?: TeamMembersAPI,
): UseMutationResult<void, Error, { teamId: string; userId: string }> {
  const queryClient = useQueryClient();
  const membersApi = useMemo(() => api ?? new TeamMembersAPI(supabase), [supabase, api]);
