...
import type { TeamAnnouncement, TeamAnnouncementInsert, TeamAnnouncementUpdate } from "../../types";
...
export function useUpdateTeamAnnouncementMutation(
  ...
  return useMutation({
    ...
    onSuccess: (updated: TeamAnnouncement) => {
      ...
      queryClient.setQueryData(announcementKeys.detail(updated.team_id, updated.id), updated);
      // キャッシュにないクエリも含めて、全てのクエリを無効化して再取得させる
      queryClient.invalidateQueries({
        queryKey: announcementKeys.lists(),
        predicate: (query) => {
