 ...
 * 大会記録一覧取得クエリ
 */
export function useRecordsQuery(supabase: SupabaseClient, options: UseRecordsQueryOptions = {}) {
  const {
    startDate,
    endDate,
    styleId,
...
export function useCreateRecordMutation(
  ...
  api?: RecordAPI,
): UseMutationResult<Record, Error, Omit<RecordInsert, "user_id">> {
  const queryClient = useQueryClient();
...
export function useUpdateRecordMutation(
  supabase: SupabaseClient,
  api?: RecordAPI,
): UseMutationResult<Record, Error, { id: string; updates: RecordUpdate }> {
  const queryClient = useQueryClient();
  const recordApi = useMemo(() => api ?? new RecordAPI(supabase), [supabase, api]);
...
export function useCreateCompetitionMutation(
  supabase: SupabaseClient,
  api?: RecordAPI,
): UseMutationResult<Competition, Error, Omit<CompetitionInsert, "user_id">> {
  const queryClient = useQueryClient();
  const recordApi = useMemo(() => api ?? new RecordAPI(supabase), [supabase, api]);
      ...
      try {
        ...
        if (user) {
          ...
          if (profile?.google_calendar_enabled && profile?.google_calendar_sync_competitions) {
            ...
            const syncResult = await syncCompetitionToGoogleCalendar(
              created,
              "create",
              undefined,
...
export function useUpdateCompetitionMutation(
  supabase: SupabaseClient,
  api?: RecordAPI,
): UseMutationResult<Competition, Error, { id: string; updates: CompetitionUpdate }> {
  const queryClient = useQueryClient();
  const recordApi = useMemo(() => api ?? new RecordAPI(supabase), [supabase, api]);
        ...
        try {
          ...
          if (user) {
            ...
            if (
              ...
              profile?.google_calendar_sync_competitions &&
              googleEventId
            ) {
              const { syncCompetitionToGoogleCalendar } = await import("../../api/google-calendar");
              // React Native用にアクセストークンを取得
              ...
              await syncCompetitionToGoogleCalendar(
                competitionData,
                "delete",
                googleEventId,
                session?.access_token,
 ...
 * スプリットタイム作成ミューテーション
 */
export function useCreateSplitTimesMutation(
  supabase: SupabaseClient,
  api?: RecordAPI,
): UseMutationResult<
  SplitTime[],
 ...
 */
export function useReplaceSplitTimesMutation(
  supabase: SupabaseClient,
  api?: RecordAPI,
): UseMutationResult<
  SplitTime[],
  Error,
  { recordId: string; splitTimes: Omit<SplitTimeInsert, "record_id">[] }
> {
  const queryClient = useQueryClient();
  const recordApi = useMemo(() => api ?? new RecordAPI(supabase), [supabase, api]);
