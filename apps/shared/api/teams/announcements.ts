...
export class TeamAnnouncementsAPI {
  ...
  async list(teamId: string, viewOnly: boolean = false): Promise<TeamAnnouncement[]> {
    ...
    let query = this.supabase.from("announcements").select("*").eq("team_id", teamId);
  ...
  async remove(id: string): Promise<void> {
    ...
    const { error } = await this.supabase.from("announcements").delete().eq("id", id);
