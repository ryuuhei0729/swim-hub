...
export class CompetitionAPI {
   ...
   * @returns 保存されたパス
   */
  async uploadCompetitionImage(competitionId: string, file: File): Promise<string> {
   ...
   */
  async uploadCompetitionImages(competitionId: string, files: File[]): Promise<string[]> {
  ...
  async deleteCompetitionImage(path: string): Promise<void> {
    ...
    const response = await fetch(
      `${appUrl}/api/storage/images/competition?path=${encodeURIComponent(path)}`,
      {
        method: "DELETE",
        credentials: "include",
      },
  ...
  getCompetitionImageUrl(path: string): string {
    ...
    const { data } = this.supabase.storage.from("competition-images").getPublicUrl(path);
