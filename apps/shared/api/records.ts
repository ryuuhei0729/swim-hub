...
export class RecordAPI {
   ...
   *
   * 個人のエントリー入力画面 (`CompetitionTabModal.tsx` / `EntryLogForm.tsx`) は、この関数と
   * 同じ形のデータ (`useBestTimes` フックが自分自身の userId で相当のクエリを行った結果) を
   * 「ベストタイムを流用」ボタンのプリフィル値にも使う。これは上記の代理入力プリフィルの制限
   * (同水路・非リレーの1件のみ) とは意図的に異なる例外: 個人画面はバッジで出所
   * (同水路/他水路・リレー有無) を明示した上でユーザー本人が1行ずつ判断して押す操作であり、
   * 押下直後は未編集警告も出る。代理入力 (管理者が出所ラベル無しで数十行を一括プリフィルする)
   * とはリスクが異なるため、他水路や引き継ぎタイムの混入を許容している。
   *
