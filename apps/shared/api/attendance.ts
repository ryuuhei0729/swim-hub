...
export class AttendanceAPI {
  ...
  async getMyAttendancesByMonth(
    ...
    // 練習の出欠情報を取得
    const { data: practiceAttendances, error: practiceError } =
    ...
    // 大会の出欠情報を取得
    const { data: competitionAttendances, error: competitionError } =
  ...
  async bulkUpdateMyAttendances(
    ...
    const updatePromises = Array.from(updateMap.entries()).map(async ([attendanceId, update]) => {
      ...
      // イベントがclosedかチェック
      const isClosed = await this.isEventClosed(existing.practice_id, existing.competition_id);
   ...
   * 出欠情報からteam_idを取得
   */
  private async getTeamIdForAttendance(attendance: TeamAttendanceInsert): Promise<string | null> {
