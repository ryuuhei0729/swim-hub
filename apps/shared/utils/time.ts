...
/**
 * 秒数を短縮形式にフォーマット（空文字対応版）
 *
 * @param seconds - 秒数
 * @returns フォーマットされた時間文字列（0の場合は空文字）
 * @example formatTimeShort(65.42) => "1:05.42"
 * @example formatTimeShort(45.67) => "45.67"
...
/**
 * 秒数を "M:SS.ms" 形式にフォーマット（常に分を表示）
 *
 * @param seconds - 秒数
 * @returns フォーマットされた時間文字列
 * @example formatTimeFull(45.67) => "0:45.67"
 * @example formatTimeFull(0) => "0:00.00"
 */
 ...
 *
 * @param timeString - 時間文字列 ("1:23.45", "23.45", "23.45s" など)
 * @returns 秒数（無効な場合は0）
 * @example parseTime("1:23.45") => 83.45
 * @example parseTime("23.45") => 23.45
 */
export function parseTime(timeString: string): number {
  ...
  // "SS.ms" や "SS.mss" 形式（末尾に 's' がある場合も対応）
  let cleaned = trimmed
