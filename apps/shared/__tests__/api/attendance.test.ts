...
describe("AttendanceAPI", () => {
  ...
  beforeEach(() => {
    ...
    api = new AttendanceAPI(supabaseMock.client);
  });
  ...
  describe("updateMyAttendance", () => {
    ...
    it("自分以外の出欠情報は更新できない", async () => {
      ...
      await expect(api.updateMyAttendance("attendance-1", { status: "present" })).rejects.toThrow(
        "自分の出欠情報のみ更新可能です",
      );
    });
  ...
  describe("updateAttendance", () => {
    ...
    it("チーム情報が取得できない場合はエラーとなる", async () => {
      supabaseMock.queueTable("team_attendance", [
        { data: { practice_id: "practice-1", competition_id: null } },
      ]);
      supabaseMock.queueTable("practices", [{ data: null }]);
