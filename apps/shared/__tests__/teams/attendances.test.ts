...
describe("TeamAttendancesAPI", () => {
  ...
  beforeEach(() => {
    ...
    api = new TeamAttendancesAPI(supabaseMock.client);
  });
  ...
  describe("listByCompetition", () => {
    ...
    it("チーム大会でない場合はエラーとなる", async () => {
      ...
      await expect(api.listByCompetition("competition-1")).rejects.toThrow(
        "チーム大会ではありません",
      );
    });
  ...
  describe("updateCompetitionAttendanceStatus", () => {
    ...
    it("大会が存在しない場合はエラーとなる", async () => {
      ...
      await expect(
        api.updateCompetitionAttendanceStatus("competition-1", "open"),
      ).rejects.toThrow();
    });
  });
});
