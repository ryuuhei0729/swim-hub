...
describe("validators", () => {
  ...
  describe("validateDistance", () => {
    ...
    it("無効な距離で失敗する", () => {
      ...
      expect(result.error).toBe(
        "距離は 25, 50, 100, 200, 400, 800, 1500 mのいずれかである必要があります",
      );
    });
  ...
  describe("validateStroke", () => {
    it.each(["freestyle", "backstroke", "breaststroke", "butterfly", "individual_medley"])(
      '有効な泳法 "%s" で成功する',
      (stroke) => {
  ...
  describe("validateAll", () => {
    it("すべて成功する場合は成功を返す", () => {
      const validations: ValidationResult[] = [{ valid: true }, { valid: true }, { valid: true }];
    ...
    it("実際のバリデーション関数と組み合わせて動作する", () => {
      const validations = [validateTime(60), validateDistance(50), validateStroke("freestyle")];
      const result = validateAll(validations);
      expect(result.valid).toBe(true);
    });
