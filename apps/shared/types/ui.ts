...
// 練習メタデータの型ガード
export function isPracticeMetadata(metadata: unknown): metadata is {
  practice?: { id: string; place: string; practice_logs?: PracticeLog[] };
  practice_id?: string;
  team_id?: string | null;
  team?: TeamInfo;
  user_id?: string;
...
// 大会メタデータの型ガード
export function isCompetitionMetadata(metadata: unknown): metadata is {
  competition?: {
    id: string;
    title: string;
    place: string | null;
...
// エントリーメタデータの型ガード
export function isEntryMetadata(metadata: unknown): metadata is {
  entry?: {
    id: string;
    competition_id: string;
    user_id: string;
  ...
  style?: { id: number; name_jp: string; distance: number };
  team_id?: string | null;
  team?: TeamInfo;
  entry_time?: number | null;
} {
...
// 記録メタデータの型ガード
export function isRecordMetadata(metadata: unknown): metadata is {
  record?: {
    time: number;
    time_result?: number;
    is_relaying: boolean;
