...
export async function getGoogleCalendarAuth(
  ...
  const { data: profile, error: profileError } = await supabase
    .from("users")
    .select(
      "google_calendar_enabled, google_calendar_sync_practices, google_calendar_sync_competitions",
    )
    .eq("id", user.id)
    .single();
  ...
  if (profileError || !profile) {
    return {
      error: NextResponse.json({ error: "ユーザー情報の取得に失敗しました" }, { status: 500 }),
    };
  }
  ...
  if (!profileData.google_calendar_enabled) {
    return {
      error: NextResponse.json(
        { error: "Google Calendar連携が有効になっていません" },
        { status: 400 },
      ),
    };
  }
  ...
  const { data: encryptedToken, error: tokenError } = await serviceClient.rpc(
  ...
  if (tokenError || !encryptedToken) {
    return {
      error: NextResponse.json(
        { error: "Google Calendar連携トークンの取得に失敗しました" },
        { status: 401 },
      ),
    };
  }
