import {
  competitionToCalendarEvent,
  fetchGoogleCalendarWithTokenRefresh,
  practiceToCalendarEvent,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const practices =
      !practicesResult.error && practicesResult.data ? (practicesResult.data as Practice[]) : [];
    const competitions =
      !competitionsResult.error && competitionsResult.data
      ...
      for (let i = 0; i < competitions.length; i += BATCH_SIZE) {
        ...
        const results = await Promise.allSettled(
          batch.map(async (competition) => {
            const teamName = competition.team_id
              ? (teamNameMap.get(competition.team_id) ?? null)
              : null;
