  ...
  try {
    ...
    if (action === "delete") {
      ...
      if (!deleteResponse.ok) {
        return NextResponse.json(
          { error: "Google Calendarへの削除に失敗しました" },
    ...
    if (action === "update" && !googleEventId) {
      return NextResponse.json({ error: "更新にはgoogleEventIdが必要です" }, { status: 400 });
    }

    const method = action === "create" ? "POST" : "PUT";
    const url = action === "update" ? `${calendarApiUrl}/${googleEventId}` : calendarApiUrl;
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: "Google Calendarへの同期に失敗しました" },
