...
export function PracticeLogTemplateList({ onCreateNew, onEdit }: PracticeLogTemplateListProps) {
  ...
  if (error) {
    return (
      <div className="text-center py-8 text-red-600">{t("list.loadError")}</div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-600">
          {t("list.savedCount", { count: count || 0, max: MAX_TEMPLATES })}
        </p>
      </div>
      ...
      {otherTemplates.length > 0 && (
        <section>
          {favoriteTemplates.length > 0 && (
            <h2 className="text-sm font-medium text-gray-500 mb-3">{t("list.allTemplatesTitle")}</h2>
          )}
          <div className="grid gap-3 sm:grid-cols-2">
      ...
      {templates && templates.length > 0 && (
        ...
        >
          <PlusIcon className="h-5 w-5" />
          {isAtLimit ? t("list.atLimitButton") : t("list.createNewButton")}
        </button>
      )}
    </div>
  );
