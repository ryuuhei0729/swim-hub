...
export async function generateMetadata({
  ...
  params: Promise<{ teamId: string; practiceId: string; locale: string }>;
}): Promise<Metadata> {
  const { practiceId, locale } = await params;
  const t = await getTranslations({ locale, namespace: "teamsAdmin" });
  ...
  const pageLabel = t("practiceLog.pageTitle");
  return {
    title: practice?.title
      ? `${practice.title} - ${pageLabel} | SwimHub`
      : `${pageLabel} | SwimHub`,
  };
}
