      ...
      <div className="px-3 py-3 sm:px-4 sm:py-4">
        <div className="mb-2 sm:mb-3">
          <h2 className="text-base sm:text-lg font-semibold text-gray-900">
            {teamMembership.team?.name} {t("announcements.title")}
          </h2>
          {teamMembership.role === "admin" && (
            <span className="text-xs text-gray-500">{t("announcements.adminRole")}</span>
          )}
        </div>
        ...
        <div className="space-y-2 sm:space-y-3">
          ...
          {teamUnansweredAttendances.length > 0 && (
            <div>
              ...
              >
                {t("announcements.unansweredAttendance", { count: teamUnansweredAttendances.length })}
              </Link>
            </div>
          )}
          ...
          {teamUnsubmittedEntries.length > 0 && (
            <div>
              <h3 className="text-sm font-medium text-gray-700 mb-2">
                {t("announcements.unsubmittedEntries", { count: teamUnsubmittedEntries.length })}
              </h3>
