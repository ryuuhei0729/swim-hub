...
export const PRACTICE_IMAGE_CONFIG = {
  ...
  THUMBNAIL_QUALITY: 0.7, // サムネイルの品質（70%）
 ...
 */
export const validatePracticeImageFile = async (
  file: File,
  ...
  const fileExtension = file.name.toLowerCase().split(".").pop();
  const isValidByExtension = ["jpg", "jpeg", "png", "webp", "heic", "heif"].includes(
    fileExtension || "",
  );
  const allowedTypes: readonly string[] = PRACTICE_IMAGE_CONFIG.ALLOWED_TYPES;
  // 空のMIMEタイプは拡張子が有効な場合のみ許可
  const isValidByType =
    allowedTypes.includes(file.type) || (file.type === "" && isValidByExtension);
  ...
  if (isHeicOrHeif(file)) {
    ...
    if (!canDecode) {
      return {
        valid: false,
        error:
          "HEIC形式はこのブラウザでサポートされていません。JPEG、PNG、WebP形式をご利用ください。",
      };
    }
  }
...
export const createImageFromFile = (file: File): Promise<HTMLImageElement> => {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      ...
      createImage(url).then(resolve).catch(reject);
    };
