...
describe("useMemberBestTimes", () => {
  ...
  describe("getBestTimeForMember", () => {
    it("存在しないメンバーIDの場合はnullを返す", () => {
      ...
      const bestTime = result.current.getBestTimeForMember("non-existent", "自由形", 50);
    ...
    it("短水路と長水路の速い方を選択する", async () => {
      ...
      const bestTime = result.current.getBestTimeForMember("member-1", "自由形", 50);
  ...
  describe("コールバックの安定性", () => {
    it("loadBestTimesForMemberは再レンダリングしても同じ参照を保つ", () => {
      const mockSupabase = createMockSupabase([], null);
      const { result, rerender } = renderHook(() => useMemberBestTimes(mockSupabase as never));
