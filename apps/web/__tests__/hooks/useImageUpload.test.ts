...
describe("useImageUpload", () => {
  ...
  beforeEach(() => {
    ...
    mockValidateFile = vi
      .fn<(file: File) => Promise<ImageValidationResult>>()
      .mockResolvedValue({ valid: true });
    mockOnImagesChange = vi.fn<(files: ImageFile[], ids: string[]) => void>();
  });
  ...
  describe("onImagesChange コールバック", () => {
    ...
    it("initialDeletedIdsが正しくdeletedIdsに反映される", async () => {
      ...
      expect(mockOnImagesChange).toHaveBeenCalledWith(
        [],
        ["pre-deleted-1", "pre-deleted-2", "existing-image-1"],
      );
    });
    ...
    it("ファイル追加と既存画像削除の組み合わせで正しいペイロードが渡される", async () => {
      ...
      expect(mockOnImagesChange).toHaveBeenLastCalledWith(addedFiles, [
        "pre-deleted-1",
        "existing-1",
      ]);
    });
