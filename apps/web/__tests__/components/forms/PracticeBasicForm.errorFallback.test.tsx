/**
 * PracticeBasicForm — 保存失敗時のエラー表示 (情報露出防止の対テスト)
 *
...
vi.mock("@apps/shared/api", () => ({
  PracticeAPI: vi.fn().mockImplementation(() => ({
    getUniquePlaces: vi.fn().mockResolvedValue([]),
  })),
}));

import PracticeBasicForm from "@/components/forms/PracticeBasicForm";

describe("PracticeBasicForm — 保存失敗時のエラー表示 — 情報露出防止の対テスト", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it(
    ...
    async () => {
      ...
      await user.click(screen.getByTestId("save-practice-button"));
