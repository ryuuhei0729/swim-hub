...
function createMockWorksheet(
  rows: Array<{ cells: Partial<Cell>[]; rowNumber: number }>,
): MockWorksheet {
...
describe("applyPracticeSheetFormatting", () => {
  ...
  it("ヘッダー行に書式が適用される", () => {
    ...
    const worksheet = createMockWorksheet([{ cells: headerCells, rowNumber: 1 }]);
  ...
  it("日付セルにExcel数値形式が処理される", () => {
    ...
    const worksheet = createMockWorksheet([
      ...
      {
        cells: [
          createMockCell(excelSerialDate),
          createMockCell("土"),
          createMockCell(""),
          createMockCell(""),
          createMockCell(""),
  ...
  it("曜日セルのフォントがグレーに設定される", () => {
    const worksheet = createMockWorksheet([
      ...
      {
        cells: [
          createMockCell(new Date(2025, 0, 15)),
          createMockCell("水"),
          createMockCell(""),
          createMockCell(""),
          createMockCell(""),
...
describe("parsePracticeExcelFile", () => {
  ...
  it("generatePracticeExcelTemplateで生成したワークブックをパースできる", async () => {
    ...
    const { generatePracticeExcelTemplate, parsePracticeExcelFile: parse } =
      await import("../../utils/practiceExcel");
  ...
  it("サンプルシートがスキップされる", async () => {
    ...
    const sampleData = result.practices.filter(
      (p) => p.place === "市民プール" || p.place === "県立プール",
    );
