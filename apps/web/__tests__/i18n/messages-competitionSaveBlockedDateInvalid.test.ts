/**
 * R3 (pool_type 破壊バグ Sprint、Critical) 対応で追加されたエラーメッセージキーの
 * 5言語パリティテスト。
 *
 * `dashboard.handlers.competitionSaveBlockedDateInvalid` は
 * CompetitionTabModal.tsx の R3 修正 (フィールド単位マージ後の日付整合性再検証。
 * マージ結果が endDate < date になる場合、部分保存せずエラーで停止しモーダルを
 * 閉じない) で使用される、新規追加のエラーメッセージキー。
 *
 * このリポジトリでは i18n キーの継ぎ目 (ある言語だけ追加漏れ) が繰り返し発生しており、
 * tsc/lint/build が全 green のまま欠落が本番まで通過した実績がある (欠落しても
 * next-intl はキー文字列をそのまま出す/フォールバックするため、型エラーにならない)。
 * そのため CI (tsc/lint/build) では検知できない前提でこの専用テストを追加する
 * (messages-competitionSaveBlockedUnresolved.test.ts と同じ観点)。
 */
...
const KEY_PATH = "dashboard.handlers.competitionSaveBlockedDateInvalid";
...
// (messages-coverage.test.ts の JA_REGEX / JA_KANA_ONLY_REGEX と同じ区別)。
...
describe("[V-I18N-R3] competitionSaveBlockedDateInvalid の5言語パリティ", () => {
  ...
  test("competitionSaveBlockedUnresolved と文言が重複していない (別エラーとして区別できる)", () => {
    const dateInvalid = getNestedValue(jaMessages as unknown as Record<string, unknown>, KEY_PATH);
    const unresolved = getNestedValue(
      jaMessages as unknown as Record<string, unknown>,
      "dashboard.handlers.competitionSaveBlockedUnresolved",
    );
    expect(dateInvalid).not.toBe(unresolved);
  });
});
