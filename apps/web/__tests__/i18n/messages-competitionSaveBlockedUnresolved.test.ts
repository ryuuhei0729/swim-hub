/**
 * Critical-2 (pool_type 破壊バグ Sprint) 対応で追加されたエラーメッセージキーの
 * 5言語パリティテスト。
 *
 * `dashboard.handlers.competitionSaveBlockedUnresolved` は
 * CompetitionTabModal.tsx の Critical-2 修正 (競技会行の再取得リトライも失敗した場合、
 * 何も書き込まずエラーを表示してモーダルを閉じない) で使用される、新規追加の
 * エラーメッセージキー。
 *
...
const KEY_PATH = "dashboard.handlers.competitionSaveBlockedUnresolved";
...
describe("[V-I18N-CRIT2] competitionSaveBlockedUnresolved の5言語パリティ", () => {
