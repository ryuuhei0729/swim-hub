...
export function useAdminAttendance(supabase: SupabaseClient, teamId: string) {
    ...
    async (eventId: string) => {
      ...
      try {
        ...
        if (event.type === "practice") {
          ...
          await attendancesAPI.updateCompetitionAttendanceStatus(
            eventId,
            editState.attendanceStatus,
          );
        }

        setEvents((prevEvents) =>
          prevEvents.map((e) =>
            e.id === eventId ? { ...e, attendance_status: editState.attendanceStatus } : e,
          ),
        );
  ...
  const handleBulkUpdate = useCallback(
    async (selectedEventIds: Set<string>, status: AttendanceStatusType) => {
        ...
        await Promise.all(
          updates.map((update) => {
            if (update.eventType === "practice") {
              ...
              return attendancesAPI.updateCompetitionAttendanceStatus(
                update.eventId,
                update.status,
              );
            }
