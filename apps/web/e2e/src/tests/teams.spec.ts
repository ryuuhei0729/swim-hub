...
test.describe("チーム機能のテスト", () => {
  ...
  test("TC-TEAMS-001: チーム作成", async ({ page }) => {
    ...
    if (!isCreateButtonVisible) {
      console.log(
        "チーム作成ボタンが表示されないため、テストをスキップします（既にチームに参加している可能性があります）",
      );
  ...
  test("TC-TEAMS-003: チーム情報編集", async ({ page }) => {
    ...
    if (await editButton.isVisible()) {
      ...
      const updateButton = page
        .locator('button[type="submit"], button:has-text("更新"), button:has-text("保存")')
        .first();
  ...
  test("TC-TEAMS-004: チーム脱退", async ({ page }) => {
    ...
    const leaveButton = page
      .locator('button:has-text("脱退"), button:has-text("チームを離れる")')
      .first();
  ...
  test("TC-TEAMS-005: チーム削除", async ({ page }) => {
    ...
    const deleteButton = page
      .locator('button:has-text("削除"), button:has-text("チームを削除")')
      .first();
