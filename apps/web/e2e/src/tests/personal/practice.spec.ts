...
test.describe("個人練習記録のテスト", () => {
  ...
  test("TC-PRACTICE-004: 練習タイムの編集", async ({ page }) => {
    ...
    // ステップ6-9: タイムを変更
    // TODO: TC-PRACTICE-004 は TC-PRACTICE-003 の実行結果に依存しています。
    // TC-PRACTICE-003 で本数3×セット数3に変更した場合、3×3=9のタイム入力フィールドがあり、
    // TC-PRACTICE-003 が先に実行されていない場合は2×2=4のフィールドがあります。
    // 理想的には test.beforeEach で独自のシードデータを作成し、テスト間の依存をなくすべきです。
    // 利用可能なフィールドにタイムを入力（シード状態 2×2 でも TC-PRACTICE-003 後の 3×3 でも対応）
    const times = [
