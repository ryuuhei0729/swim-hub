...
test.describe("個人練習記録のテスト", () => {
  ...
  test("TC-PRACTICE-001: 練習記録の追加（基本フロー）", async ({ page }) => {
    ...
    await page.waitForSelector(
      `[data-testid="practice-menu-container"]:nth-child(${menuCountBefore + 1})`,
    ...
    if (menuCountAfter > 1) {
      ...
      await page
        .waitForSelector(`[data-testid="practice-menu-container"]:nth-child(${menuCountAfter})`, {
          state: "detached",
          timeout: 5000,
        })
        .catch(() => {});
      const menuCountAfterDelete = await page
        .locator('[data-testid="practice-menu-container"]')
        .count();
  ...
  test("TC-PRACTICE-003: プラクティスログの編集（サークル・本数・セット数など）", async ({
    ...
    if (editLogButtonCount === 0) {
      throw new Error(
        "練習ログが見つかりません。TC-PRACTICE-001で練習ログが作成されていることを確認してください。",
      );
    }
