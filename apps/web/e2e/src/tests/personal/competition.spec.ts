...
test.describe("個人大会記録のテスト", () => {
  ...
  test("TC-COMPETITION-001: 大会記録の追加（スプリットタイムあり・リレー種目）", async ({
    ...
    await page.waitForSelector(
      '[data-testid="day-detail-modal"], [data-testid="practice-detail-modal"], [data-testid="record-detail-modal"]',
      { timeout: 5000 },
    );
    ...
    await page.waitForSelector('[data-testid="record-split-distance-1-1"]', {
      state: "visible",
      timeout: 5000,
    });
    ...
    await page.waitForSelector('[data-testid="record-split-distance-1-2"]', {
      state: "visible",
      timeout: 5000,
    });
    ...
    await page.waitForSelector('[data-testid="record-split-distance-1-3"]', {
      state: "visible",
      timeout: 5000,
    });
  ...
  test("TC-COMPETITION-006: エントリーの削除（未来の大会のみ）", async ({ page }) => {
    ...
    if (!hasEntrySummary) {
      // 今日の日付ではエントリーではなくレコードが表示されるため、テストをスキップ
      console.log(
        "今日の日付ではエントリー削除は利用できません（レコードフローが使用されます）。テストをスキップします。",
      );
      // 代わりにレコードが表示されていることを確認
      const recordDisplay = page.locator('[data-testid="record-time-display"]').first();
      const hasRecord = await recordDisplay.isVisible({ timeout: 2000 }).catch(() => false);
