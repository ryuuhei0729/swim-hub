...
test.describe("メンバー管理のテスト", () => {
  ...
  test("TC-MEMBER-002: メンバー詳細表示", async ({ page }) => {
    ...
    const memberRow = page
      .locator('tr[class*="cursor-pointer"], [role="row"][class*="cursor-pointer"]')
      .first();
    ...
    const hasContent =
      (await profileSection.isVisible().catch(() => false)) ||
    ...
    // ステップ7: モーダルが閉じることを確認
    await page.waitForSelector('[data-testid="team-member-detail-modal"]', {
      state: "hidden",
      timeout: 5000,
    });
  });
  ...
  test("TC-MEMBER-003: 役割変更", async ({ page }) => {
    ...
    const memberRows = page.locator(
      'tr[class*="cursor-pointer"], [role="row"][class*="cursor-pointer"]',
    );
    ...
    if (!hasRoleToggle) {
      console.log(
        "役割変更トグルが表示されないため、テストをスキップします（自分のプロフィールを開いた可能性があります）",
      );
      // モーダルを閉じる
      const closeButton = page.locator('[data-testid="team-member-detail-close-button"]');
      await closeButton.click();
    ...
    // ステップ7: 現在の役割を取得
    const isCurrentlyAdmin = await adminButton.evaluate(
      (el) => el.className.includes("bg-yellow") || el.className.includes("shadow"),
    );
    ...
    if (hasConfirmModal) {
      ...
      const confirmButton = confirmModal
        .locator('button:has-text("確認"), button:has-text("変更"), button:has-text("はい")')
        .first();
  ...
  test("TC-MEMBER-004: メンバー削除", async ({ page }) => {
    ...
    if (!hasRemoveButton) {
      console.log(
        "削除ボタンが表示されないため、テストをスキップします（自分のプロフィールを開いた可能性があります）",
      );
      // モーダルを閉じる
      const closeButton = page.locator('[data-testid="team-member-detail-close-button"]');
      await closeButton.click();
