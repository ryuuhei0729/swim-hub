...
function getLocalSupabaseEnv(): SupabaseEnv | null {
  try {
    ...
    const result = execSync("pnpm exec supabase status --workdir supabase -o json", {
    ...
    return {
      ...
      anonKey: parsed.ANON_KEY ?? parsed.PUBLISHABLE_KEY ?? DEFAULT_LOCAL_SUPABASE.anonKey,
      serviceRoleKey:
        parsed.SERVICE_ROLE_KEY ?? parsed.SECRET_KEY ?? DEFAULT_LOCAL_SUPABASE.serviceRoleKey,
    };
  } catch (error) {
    console.warn(
...
const supabaseEnvEntries =
  shouldUseLocalSupabase && localSupabaseEnv
    ? {
