...
export function FileUpload({ selectedFile, loading, onFileSelect }: FileUploadProps) {
  ...
  return (
    <div className="mb-6 p-4 bg-gray-50 rounded-lg">
      <h3 className="text-lg font-medium text-gray-900 mb-4">{t("fileUpload.title")}</h3>
      ...
      <p className="mt-2 text-sm text-gray-500">{t("fileUpload.hint")}</p>
      {loading && <p className="mt-2 text-sm text-gray-600">{t("fileUpload.loading")}</p>}
    </div>
  );
}
