...
export const useMemberBestTimes = (supabase: SupabaseClient) => {
   ...
   * 個別メンバーのベストタイムを取得
   */
  const loadBestTimesForMember = useCallback(
    async (userId: string): Promise<BestTime[]> => {
      try {
   ...
   * 複数メンバーのベストタイムを並列取得
   */
  const loadAllBestTimes = useCallback(
    async (members: Array<{ id: string; user_id: string }>) => {
      try {
        setLoading(true);
        setError(null);
