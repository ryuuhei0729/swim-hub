...
export const useRecentAttendance = (
  ...
  const loadRecentAttendances = useCallback(
    ...
    async (eventId: string) => {
      ...
      try {
        ...
        if (existingAttendance) {
          ...
          setAttendances((prev) =>
            prev.map((a) =>
              a.id === existingAttendance.id ? { ...a, status: editState.status, note } : a,
            ),
          );
          setEditStates((prev) => ({
            ...prev,
          ...
          if (selectError) {
            ...
            const isValidAttendance = (data: unknown): data is TeamAttendanceWithDetails => {
