...
export const getStatusBadge = (status: "open" | "closed" | null | undefined) => {
  switch (status) {
    ...
    case "closed":
      return (
        <span className="text-xs px-2 py-0.5 rounded-full bg-red-200 text-red-800">提出締切</span>
      );
    default:
      return (
        <span className="text-xs px-2 py-0.5 rounded-full bg-gray-200 text-gray-700">未設定</span>
      );
  }
};
