...
export default function TeamTimeInputModal({
  ...
  const handleTimeConfirm = useCallback(
    (memberId: string, timeIndex: number, value: string) => {
  ...
  return (
    <div className="fixed inset-0 z-70 overflow-y-auto" data-testid="team-time-input-modal">
      <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div
          className="relative inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-2xl border-2 border-gray-300 transform transition-all sm:my-8 sm:align-middle sm:max-w-6xl sm:w-full"
          data-testid="team-time-input-dialog"
        >
          {/* ヘッダー */}
          ...
          {/* タイム入力テーブル */}
          <div
            className="bg-white px-6 py-4 max-h-96 overflow-y-auto"
            data-testid="team-time-input-table"
          >
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {teamMembers.map((member, memberIndex) => {
                    ...
                    return (
                      <tr
                        key={member.id}
                        className={memberIndex % 2 === 0 ? "bg-white" : "bg-gray-50"}
                      >
                          ...
                          return (
                            <td key={timeIndex} className="px-3 py-3 text-center">
                              <input
                                ...
                                onBlur={(e) =>
                                  handleTimeConfirm(member.id, timeIndex, e.target.value)
                                }
                                onKeyDown={(e) =>
                                  handleKeyDown(
                                    e,
                                    ...
                                    member.id,
                                    timeIndex,
                                    memberTimes[timeIndex]?.displayValue || "",
                                  )
                                }
