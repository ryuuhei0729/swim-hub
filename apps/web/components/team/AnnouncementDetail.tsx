      ...
      <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
        <div className="bg-white rounded-lg p-6 w-full max-w-2xl mx-4">
          <div className="text-center py-8">
            <p className="text-red-600 mb-4">{t("announcementDetail.loadError")}</p>
            <button
              onClick={onClose}
              className="bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-gray-700"
            >
              {t("announcementDetail.closeButton")}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
        <div className="bg-white rounded-lg w-full max-w-3xl mx-4 max-h-[90vh] overflow-hidden">
          ...
          <div className="flex justify-between items-center p-6 border-b border-gray-200">
            <h2 className="text-xl font-semibold text-gray-900">{t("announcementDetail.title")}</h2>
            <div className="flex gap-2">
              {isAdmin && onEdit && (
                ...
                >
                  {t("announcementDetail.editButton")}
                </button>
              )}
              {isAdmin && (
                <button
                ...
                >
                  {deleteAnnouncementMutation.isPending
                    ...
                    : t("announcementDetail.deleteButton")}
                </button>
              )}
              <button
                onClick={onClose}
            ...
            <div className="mb-6">
              ...
              <div className="flex items-center gap-4 text-sm text-gray-500">
                {!announcement.is_published && (
                  <span className="bg-yellow-100 text-yellow-800 px-2 py-1 rounded text-xs">
                    {t("announcementDetail.draftBadge")}
                  </span>
                )}
                <span>{t("announcementDetail.createdAtLabel")} {formatDateTime(announcement.created_at)}</span>
                {announcement.updated_at !== announcement.created_at && (
                  <span>{t("announcementDetail.updatedAtLabel")} {formatDateTime(announcement.updated_at)}</span>
                )}
              </div>
              ...
              {(announcement.start_at || announcement.end_at) && (
                <div className="mt-3 p-3 bg-gray-50 rounded-lg">
                  <p className="text-sm font-medium text-gray-700 mb-1">{t("announcementDetail.periodLabel")}</p>
                  <div className="text-sm text-gray-600">
                    {announcement.start_at ? (
                      <span>{t("announcementDetail.startLabel")} {formatDateTime(announcement.start_at)}</span>
                    ) : (
                      <span>{t("announcementDetail.startLabel")} {t("announcementDetail.noLimit")}</span>
                    )}
                    {announcement.start_at && announcement.end_at && <span className="mx-2">〜</span>}
                    {announcement.end_at ? (
                      <span>{t("announcementDetail.endLabel")} {formatDateTime(announcement.end_at)}</span>
                    ) : (
                      <span>{t("announcementDetail.endLabel")} {t("announcementDetail.noLimit")}</span>
                    )}
                  </div>
                  {/* 期間外の場合は警告表示 */}
                  {announcement.is_published &&
      ...
      <ConfirmDialog
        ...
        title={t("announcementDetail.deleteConfirm.title")}
        message={t("announcementDetail.deleteConfirm.message")}
        confirmLabel={t("announcementDetail.deleteConfirm.confirmButton")}
        cancelLabel={t("announcementDetail.deleteConfirm.cancelButton")}
        variant="danger"
      />
    </>
  );
