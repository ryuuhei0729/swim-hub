...
export default function TeamBulkRegister({ teamId, isAdmin = false }: TeamBulkRegisterProps) {
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      ...
      {/* ファイルアップロード */}
      <FileUpload selectedFile={selectedFile} loading={loading} onFileSelect={handleFileSelect} />

      {/* プレビュー */}
      {parsedData && (
        <DataPreview parsedData={parsedData} loading={loading} onRegister={handleBulkRegister} />
      )}
