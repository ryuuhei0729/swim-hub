...
export interface CompetitionImageFile {
  id: string
  file: File
  previewUrl: string
}
...
interface CompetitionImageUploaderProps {
  ...
  onImagesChange: (newFiles: CompetitionImageFile[], deletedIds: string[]) => void
  disabled?: boolean
}
...
export default function CompetitionImageUploader({
  ...
  const [newFiles, setNewFiles] = useState<CompetitionImageFile[]>([])
  ...
  const canAddMore = totalImageCount < COMPETITION_IMAGE_CONFIG.MAX_IMAGES
  ...
  const handleFiles = useCallback(async (files: FileList | File[]) => {
    ...
    // 最大枚数チェック
    const remainingSlots = COMPETITION_IMAGE_CONFIG.MAX_IMAGES - totalImageCount
    if (fileArray.length > remainingSlots) {
      setError(`あと${remainingSlots}枚まで追加できます（最大${COMPETITION_IMAGE_CONFIG.MAX_IMAGES}枚）`)
      return
    }

    const validFiles: CompetitionImageFile[] = []
    
    for (const file of fileArray) {
      const validation = await validateCompetitionImageFile(file)
      if (!validation.valid) {
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-gray-500">
          {totalImageCount} / {COMPETITION_IMAGE_CONFIG.MAX_IMAGES}枚
        </span>
      </div>
      ...
      {!canAddMore && (
        <p className="text-sm text-gray-500 text-center py-2">
          最大{COMPETITION_IMAGE_CONFIG.MAX_IMAGES}枚の画像が選択されています
        </p>
      )}
    </div>
  )
