...
export const TeamEntryBulkFormScreen: React.FC = () => {
  ...
  const [existingEntryRows, setExistingEntryRows] = useState<
    ExistingEntryRow[]
  >([]);
  ...
  const memberNameByUserId = useMemo(() => {
    ...
    members.forEach((m) =>
      map.set(m.user_id, m.users?.name || t("teams.mobile.unnamedMember")),
    );
    ...
    const load = async () => {
      try {
        ...
        const initialDraftRows: EntryDraftRow[] = rawEntries.map((e) => ({
          ...
          styleId: e.style_id,
          entryTimeInput:
            e.entry_time != null ? formatTimeBest(e.entry_time) : "",
          note: e.note || "",
          prefillSource: null,
          prefilledInput: null,
        }));
  ...
  useEffect(() => {
    ...
    const loadBestTimes = async () => {
      try {
        ...
        const map = await recordApi.getBestTimesForUsers(
          userIds,
  ...
  const validDraftRows = useMemo(
    () =>
      draftRows.filter(
        (r): r is EntryDraftRow & { styleId: number } => r.styleId !== "",
      ),
    [draftRows],
  );
  ...
  const conflictingDeleteRows = useMemo(() => {
    const { conflicting } = partitionConflictingDeletes(
      diffResult,
      existingEntryRows,
      validDraftRows,
    );
  ...
  const showConflictingDeleteAlert = () => {
    const list = conflictingDeleteRows
      .map(
        (e) =>
          `・${memberNameByUserId.get(e.user_id) ?? ""} ${styleNameById(e.style_id)}`,
      )
      .join("\n");
    Alert.alert(
  ...
  const changedFromSnapshot = useMemo(() => {
    ...
    return hasUnsavedChanges(
      draftRows,
      JSON.parse(snapshotRef.current) as EntryDraftRow[],
    );
  }, [draftRows]);
  ...
  const getBestTimeForRow = (
    targetUserId: string,
    styleId: number | "",
  ): BestTime | undefined => {
    if (styleId === "") return undefined;
    return bestTimesByUserId
      .get(targetUserId)
      ?.find((bt) => bt.style_id === styleId);
  };
  ...
  const confirmMemberSelection = (selectedUserIds: string[]) => {
    setDraftRows((prev) => {
      ...
      const kept = prev.filter(
        (r) =>
          selectedUserIds.includes(r.targetUserId) ||
          r.existingEntryId !== null,
      );
  ...
  const clearRowStyle = (localId: string) => {
    setDraftRows((prev) =>
      prev.map((r) => (r.localId === localId ? { ...r, styleId: "" } : r)),
    );
  };
  ...
  const updateRowTimeInput = (localId: string, text: string) => {
    setDraftRows((prev) =>
      prev.map((r) =>
        r.localId === localId ? { ...r, entryTimeInput: text } : r,
      ),
    );
  };
    ...
    setDraftRows((prev) =>
      prev.map((r) => {
        ...
        if (parsed === null) {
          setErrors((prevErrors) => ({
            ...prevErrors,
            [localId]: t("teams.mobile.entryBulk.timeFormatInvalid"),
          }));
          return r;
        }
        setErrors((prevErrors) => {
  ...
  const updateRowNote = (localId: string, text: string) => {
    setDraftRows((prev) =>
      prev.map((r) => (r.localId === localId ? { ...r, note: text } : r)),
    );
  };
  ...
  const handleOpenConfirm = () => {
    ...
    if (!hasAnyChange) {
      Alert.alert(
        t("common.error"),
        t("teams.mobile.entryBulk.noValidRowsAlert"),
      );
      return;
    }
  ...
  const handleConfirmSave = async () => {
    ...
    try {
      ...
      Alert.alert(
        t("common.error"),
        isUniqueViolation
          ? t("teams.mobile.entryBulk.saveFailedDuplicate")
          : t("recordMobile.saveFailed"),
      );
    } finally {
  ...
  if (loading || membersLoading) {
    return (
      <View style={styles.container}>
        <LoadingSpinner
          fullScreen
          message={t("competition.mobile.entryLoading")}
        />
      </View>
    );
  }
    ...
    return (
      <View style={styles.container}>
        <View style={styles.permissionContainer}>
          ...
          <Text style={styles.permissionText}>
            {t("teams.mobile.entryBulk.pastDateWarning")}
          </Text>
          <Pressable
            style={styles.permissionButton}
            onPress={() => navigation.goBack()}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <View style={styles.compHeader}>
          ...
          <Text style={styles.compSubtitle}>
            {t("teams.mobile.entryBulk.description")}
          </Text>
        ...
        >
          ...
          <Text style={styles.addMemberButtonText}>
            {t("teams.mobile.entryBulk.addMemberButton")}
          </Text>
        </Pressable>
        ...
        {memberOrder.length === 0 && (
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>
              {t("teams.mobile.entryBulk.emptyMembersHint")}
            </Text>
          </View>
        )}
          ...
          return (
            <View key={userId} style={styles.memberCard}>
              ...
              {rows.map((row, rowIndex) => {
                const selectedStyle =
                  row.styleId !== "" ? styleById.get(row.styleId) : undefined;
                const bestTime = getBestTimeForRow(
                  row.targetUserId,
                  row.styleId,
                );
                const isDuplicate =
                  ...
                  <View key={row.localId} style={styles.rowCard}>
                    ...
                    <View style={styles.field}>
                      ...
                      <TextInput
                        ...
                        onChangeText={(text) =>
                          updateRowTimeInput(row.localId, text)
                        }
                        onBlur={() => handleTimeBlur(row.localId)}
                        placeholder={t(
                          "teams.mobile.entryBulk.timePlaceholder",
                        )}
              ...
              {memberActive && (
                <Pressable
                  ...
                  onPress={() => addRowForMember(userId)}
                >
                  <Feather name="plus" size={14} color="#2563EB" />
                  <Text style={styles.addEventText}>
                    {t("teams.mobile.entryBulk.addEventButton")}
                  </Text>
                </Pressable>
              )}
            </View>
        ...
        >
          {saving ? (
            ...
            <Text style={styles.saveButtonText}>
              {t("teams.mobile.entryBulk.saveButton")}
            </Text>
          )}
        </Pressable>
      </SafeAreaView>
        ...
        <ScrollView>
          ...
          {swimStyles.map((style) => (
            <Pressable
              ...
              onPress={() => {
                if (stylePickerRowId)
                  updateRowStyle(stylePickerRowId, style.id);
                setStylePickerRowId(null);
              }}
            >
      ...
      >
        <View style={styles.pickerSheetHeader}>
          <Text style={styles.pickerSheetTitle}>
            {t("teams.mobile.entryBulk.confirmModalTitle")}
          </Text>
          <Pressable
            ...
            <View style={styles.confirmSection}>
              <Text
                style={[
                  ...
                  styles.confirmSectionTitleNew,
                ]}
              >
                {t("teams.mobile.entryBulk.categoryNew")} (
                ...
                return (
                  <View key={`create-${idx}`} style={styles.confirmRow}>
                    <Text style={styles.confirmRowText}>
                      {memberNameByUserId.get(item.user_id) ?? ""} ・{" "}
                      {styleNameById(item.style_id)}
                      {" → "}
          ...
          {diffResult.toUpdate.length > 0 && (
            <View style={styles.confirmSection}>
              <Text
                style={[
                  styles.confirmSectionTitle,
                  styles.confirmSectionTitleUpdate,
                ]}
              >
                {t("teams.mobile.entryBulk.categoryUpdated")} (
                {diffResult.toUpdate.length})
              </Text>
          ...
          {diffResult.toDelete.length > 0 && (
            <View style={styles.confirmSection}>
              <Text
                style={[
                  styles.confirmSectionTitle,
                  styles.confirmSectionTitleDelete,
                ]}
              >
                {t("teams.mobile.entryBulk.categoryDeleted")} (
              ...
              {diffResult.toDelete.map((id) => {
                ...
                return (
                  <View key={`delete-${id}`} style={styles.confirmRow}>
                    <Text
                      style={[
                        styles.confirmRowText,
                        styles.confirmRowTextDeleted,
                      ]}
                    >
                      {memberNameByUserId.get(existing.user_id) ?? ""} ・{" "}
                      {styleNameById(existing.style_id)}
          ...
          {diffResult.unchanged.length > 0 && (
            <View style={styles.confirmSection}>
              <Text style={styles.confirmSectionTitle}>
                {t("teams.mobile.entryBulk.categoryUnchanged")} (
        ...
        <SafeAreaView edges={["bottom"]} style={styles.confirmFooter}>
          ...
          >
            {saving ? (
              ...
              <Text style={styles.saveButtonText}>
                {t("teams.mobile.entryBulk.confirmButton")}
              </Text>
            )}
          </Pressable>
        </SafeAreaView>
