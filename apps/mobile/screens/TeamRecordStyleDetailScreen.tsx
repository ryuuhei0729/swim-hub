...
import type { MemberRecord, StyleEntry, SplitTimeEntry } from "./teamRecordBulk/buildStyleEntries";
import {
...
function buildEmptyIndividualEntry(styleId: number, styleName: string): StyleEntry {
...
export const TeamRecordStyleDetailScreen: React.FC = () => {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const resolvedList = relayEventId
        ...
        : styleId !== undefined
          ? result.styleEntries.filter((e) => !e.relayEventId && e.styleId === styleId)
          : [];

      const relayDef = relayEventId ? relayEvents.find((r) => r.id === relayEventId) : undefined;
        ...
        resolvedList.length > 0
          ...
          : [
              relayEventId && relayDef
                ? buildEmptyRelayEntry(relayEventId, relayDef, isPremiumRef.current)
                : styleId !== undefined
      ...
      existingRelayRecordIdsRef.current = relayEventId
        ? await scopeRelayRecordIdsForLegRecords(supabase, Array.from(existingRecordIdsRef.current))
        : new Set();
  ...
  const changedFromSnapshot = useMemo(() => {
    ...
    return hasUnsavedChanges(entries, JSON.parse(snapshotRef.current) as StyleEntry[]);
  }, [entries]);
  ...
  const setEntry = (updater: (prev: StyleEntry | null) => StyleEntry | null) => {
  ...
  // ---- 組 (StyleEntry) の追加・削除 ----
  const addGroup = () => {
    const relayDef = relayEventId != null ? relayEvents.find((r) => r.id === relayEventId) : undefined;
    ...
    setEntries((prev) => {
      ...
      if (relayEventId != null && relayDef) {
        newEntry = buildEmptyRelayEntry(relayEventId, relayDef, isPremiumRef.current);
      } else {
        ...
        newEntry = buildEmptyIndividualEntry(individualStyleId, template?.styleName ?? "");
      }
  ...
  const confirmMemberSelection = (selectedUserIds: string[]) => {
    setEntry((prev) => {
      ...
      for (const userId of selectedUserIds) {
        const existing = prev.memberRecords.find((mr) => mr.memberUserId === userId);
        if (existing) {
      ...
      return { ...prev, memberRecords: newMemberRecords };
    });
    setMemberModalOpen(false);
  };

  const updateMemberRecordByIndex = (legIndex: number, updates: Partial<MemberRecord>) => {
    setEntry((prev) =>
      prev
        ? {
            ...prev,
  ...
  // まとめて更新されてしまう (事実4)。
  const updateMemberRecord = (recordId: string, updates: Partial<MemberRecord>) => {
    setEntry((prev) =>
      prev
        ? {
            ...prev,
  ...
  const handleReactionTimeBlurByIndex = (legIndex: number, value: string) => {
    updateMemberRecordByIndex(legIndex, { reactionTime: normalizeReactionTime(value) });
  };

  const handleReactionTimeBlur = (recordId: string, value: string) => {
    updateMemberRecord(recordId, { reactionTime: normalizeReactionTime(value) });
  };
          ...
          if (raceDistance && newTime > 0) {
            ...
            if (idx >= 0) {
              updatedSplitTimes = updatedSplitTimes.map((st, i) =>
                i === idx ? { ...st, splitTime: newTime, displayValue: formatTimeBest(newTime) } : st,
              );
            } else {
              updatedSplitTimes = [
                ...updatedSplitTimes,
  ...
  const countBillableSplitTimes = (splitTimes: SplitTimeEntry[]): number => {
    ...
    return splitTimes.filter((st) => !(typeof st.distance === "number" && st.distance === raceDistance))
      .length;
  };
  ...
  const handleRelayTotalTimeChange = (value: string) => {
    setEntry((prev) => {
      ...
      const updatedMemberRecords = prev.memberRecords.map((mr, idx) => {
        ...
        const cumTime = isLastLeg ? totalSeconds : newCum > 0 ? newCum : (mr.cumulativeTimeSeconds ?? 0);
      ...
      return { ...prev, relaySplitTimes: updatedSplits, memberRecords: updatedMemberRecords };
    });
  };
  ...
  const addRelaySplitTimesAtInterval = (interval: number) => {
    setEntry((prev) => {
      ...
      if (totalDistance === undefined) return prev; // legBoundaries は常に4要素だが防御的に扱う
      const currentSplits = prev.relaySplitTimes ?? [];
      const existingDistances = new Set(currentSplits.map((st) => st.distance).filter((d) => d > 0));
  ...
  const addRelaySplitTime = () => {
    setEntry((prev) => {
      if (!prev || !prev.relayEventId) return prev;
      const currentSplits = prev.relaySplitTimes ?? [];
      if (!isPremium && currentSplits.length >= RELAY_FREE_PLAN_MAX_SPLITS) return prev;
      return {
        ...prev,
        relaySplitTimes: [...currentSplits, { id: genId(), distance: 0, splitTime: 0, displayValue: "" }],
      };
    });
  };
  ...
  const removeRelaySplitTime = (splitId: string) => {
    setEntry((prev) =>
      !prev || !prev.relayEventId
        ...
        : {
            ...prev,
            relaySplitTimes: (prev.relaySplitTimes ?? []).filter((st) => st.id !== splitId),
          },
    );
  };
    ...
    setEntry((prev) => {
      ...
      return {
        ...
        memberRecords: prev.memberRecords.map((mr) => {
          ...
          return {
            ...mr,
            splitTimes: [...mr.splitTimes, { id: genId(), distance: 0, splitTime: 0, displayValue: "" }],
          };
        }),
      };
    });
      ...
      return {
        ...
        memberRecords: prev.memberRecords.map((mr) => {
          ...
          const existingDistances = new Set(
            mr.splitTimes.map((st) => (typeof st.distance === "number" ? st.distance : 0)).filter((d) => d > 0),
          );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.videoErrors.length > 0) {
        const messages = result.videoErrors.map((videoError: StyleRecordVideoError) => {
        ...
        Alert.alert(
          t("common.notice"),
          t("teamsAdmin.practiceLog.videoPartialFailureSaved", { errors: messages.join("\n") }),
        );
      }
      ...
      Alert.alert(t("common.error"), toUserFacingMessage(err, t("competition.records.error.saveFailed")));
    } finally {
      setSaving(false);
    }
  };
  ...
  const relayDef = entry.relayEventId ? relayEvents.find((r) => r.id === entry.relayEventId) : undefined;
  ...
  return (
    <FormKeyboardAvoidingView style={styles.container}>
      ...
      >
        ...
        {/* 「組」の切り替え (1本目/2本目/... リレーならAチーム/Bチームに相当)。
            予選・決勝や同一種目の複数チーム分の入力を、種目1つ=1タブとして許容する (事実4)。
            組の区別は番号のみで、予選/決勝の書き分けは既存の備考欄に委ねる (migration 不要)。 */}
        <ItemTabs
          count={entries.length}
          activeIndex={activeGroupIndex}
          onSelect={setActiveGroupIndex}
          onAdd={addGroup}
          onRemove={entries.length > 1 ? removeGroup : undefined}
          label={(i) => t("teams.record.groupNumber", { n: i + 1 })}
          accent="blue"
          disabled={saving}
        ...
        >
          ...
          {!entry.relayEventId && (
            <View style={styles.field}>
              ...
              <Pressable style={styles.selectMemberButton} onPress={() => setMemberModalOpen(true)}>
                <Feather name="users" size={16} color="#2563EB" />
                <Text style={styles.selectMemberText}>{t("teams.record.selectMemberButton")}</Text>
              </Pressable>
            ...
            <View style={styles.relaySection}>
              ...
              {entry.memberRecords.map((mr, mrIndex) => (
                <View key={`leg-${mrIndex}`} style={styles.legCard}>
                  ...
                  >
                    <Text style={[styles.pickerButtonText, !mr.memberUserId && styles.placeholder]}>
                      {mr.memberName || t("teams.record.relaySwimmerPlaceholder")}
                    </Text>
                    <Feather name="chevron-down" size={18} color="#6B7280" />
                  </Pressable>
                  {(() => {
                    const best = bestTimeBadgeFor(mr.memberUserId, mr.relayLegStyleId, mr.isRelaying);
                    if (!best) return null;
                    return (
                      <View testID={`relay-leg-best-time-badge-${mrIndex}`} style={styles.bestTimeBadge}>
                  ...
                  <View style={styles.rtField}>
                    <Text style={styles.smallLabel}>{t("recordMobile.form.reactionTimeLabel")}</Text>
                    <TextInput
                      style={styles.input}
                      value={mr.reactionTime || ""}
                      onChangeText={(text) => updateMemberRecordByIndex(mrIndex, { reactionTime: text })}
                      onBlur={() => handleReactionTimeBlurByIndex(mrIndex, mr.reactionTime || "")}
                      placeholder="0.65"
                ...
                <View style={styles.splitTimeHeader}>
                  ...
                  <View style={styles.splitTimeButtons}>
                    <Pressable
                      ...
                      disabled={
                        !isPremium && (entry.relaySplitTimes ?? []).length >= RELAY_FREE_PLAN_MAX_SPLITS
                      }
                    >
                    ...
                    >
                      <Text style={styles.addSplitButtonText}>{t("recordMobile.form.addButton")}</Text>
                    </Pressable>
                  </View>
                </View>
                  ...
                  .map((split) => (
                    <View key={split.id} style={styles.splitRow}>
                      <TextInput
                        ...
                        onChangeText={(text) => {
                          if (text === "" || /^\d+(\.\d*)?$/.test(text)) {
                            handleRelaySplitTimeChange(split.id, "distance", text);
                          }
                        }}
                      ...
                      <TextInput
                        ...
                        onChangeText={(text) => handleRelaySplitTimeChange(split.id, "splitTime", text)}
                        placeholder={t("teams.record.splitTimePlaceholder")}
                        keyboardType="decimal-pad"
                      />
                      <Pressable style={styles.removeSplitBtn} onPress={() => removeRelaySplitTime(split.id)}>
            ...
            entry.memberRecords.map((mr) => (
              <View key={mr.id} style={styles.memberCard}>
                ...
                {mr.entryTimeReference != null && mr.entryTimeReference > 0 && (
                  <View style={styles.entryTimeBadge}>
                    <Text style={styles.entryTimeBadgeText}>
                      {t("forms.recordLog.entryTimeLabel")} {formatTimeBest(mr.entryTimeReference)}
                    </Text>
                  </View>
                )}
                ...
                {(() => {
                  ...
                  if (!best) return null;
                  return (
                    <View testID={`record-best-time-badge-${mr.memberUserId}`} style={styles.bestTimeBadge}>
                      <Text style={styles.bestTimeBadgeText}>
                        {best.label}: {formatTimeBest(best.time)}
                      </Text>
                    </View>
                ...
                <View style={styles.timeReactionRow}>
                  ...
                  <View style={styles.reactionTimeField}>
                    ...
                    <TextInput
                      style={styles.input}
                      value={mr.reactionTime || ""}
                      onChangeText={(text) => updateMemberRecord(mr.id, { reactionTime: text })}
                      onBlur={() => handleReactionTimeBlur(mr.id, mr.reactionTime || "")}
                      placeholder="0.65"
                      keyboardType="decimal-pad"
                    />
                  </View>
                  ...
                  <View style={styles.relayField}>
                    ...
                    <View style={styles.relaySwitchRow}>
                      <Switch
                        value={mr.isRelaying}
                        onValueChange={(v) => updateMemberRecord(mr.id, { isRelaying: v })}
                      />
                    </View>
                  </View>
                </View>
                ...
                <View style={styles.field}>
                  <View style={styles.splitTimeHeader}>
                    ...
                    <View style={styles.splitTimeButtons}>
                      <Pressable
                        style={[styles.addSplitButton, !hasDistance && styles.addButtonDisabled]}
                      ...
                      <Pressable style={styles.addSplitButton} onPress={() => addSplitTime(mr.id)}>
                        <Text style={styles.addSplitButtonText}>
                    ...
                    .map((split) => (
                      <View key={split.id} style={styles.splitRow}>
                        <TextInput
                          ...
                          onChangeText={(text) => {
                            if (text === "" || /^\d+(\.\d*)?$/.test(text)) {
                              updateSplitTime(mr.id, split.id, "distance", text);
                            }
                          }}
                          placeholder={t("teams.record.splitDistancePlaceholder")}
                          keyboardType="decimal-pad"
                        ...
                        <TextInput
                          style={[styles.input, styles.splitTime]}
                          value={split.displayValue}
                          onChangeText={(text) => updateSplitTime(mr.id, split.id, "splitTime", text)}
                          placeholder={t("teams.record.splitTimePlaceholder")}
                          keyboardType="decimal-pad"
                        />
                        <Pressable
                  ...
                  {!isPremium &&
                    countBillableSplitTimes(mr.splitTimes) >= FREE_PLAN_LIMITS.SPLIT_TIMES_PER_RECORD && (
                ...
                <View style={styles.field}>
                  ...
                  <TextInput
                    style={styles.input}
                    value={mr.note}
                    onChangeText={(text) => updateMemberRecord(mr.id, { note: text })}
                ...
                <View style={styles.field}>
                  ...
                  {isPremium ? (
                    <VideoUploader
                      ...
                      isPremium={isPremium}
                      existingVideoPath={
                        data.existingRecords.find((r) => r.id === mr.id)?.video_path ?? null
                      }
                      existingThumbnailPath={
                        data.existingRecords.find((r) => r.id === mr.id)?.video_thumbnail_path ?? null
                      }
                      onPendingVideoAsset={(asset) => updateMemberRecord(mr.id, { videoAsset: asset })}
                    />
                  ) : (
      ...
      >
        <View style={styles.pickerSheetHeader}>
          <Text style={styles.pickerSheetTitle}>{t("teams.record.relaySwimmerPlaceholder")}</Text>
...
const styles = StyleSheet.create({
  ...
  compSubtitle: { fontSize: 14, color: "#2563EB", marginTop: 4, fontWeight: "600" },
