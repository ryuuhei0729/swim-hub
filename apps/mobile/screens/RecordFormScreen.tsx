...
export const RecordFormScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (record) {
      ...
      setStandaloneOriginalPoolType((record.pool_type ?? 0) as PoolType);
      // 動画パスを初期化
  ...
  return (
    ...
    >
            ...
            {sortedSplitIndices.map(({ st, index: originalIndex }) => (
              <View key={originalIndex} style={styles.splitTimeRow}>
                ...
                <TextInput
                  ...
                  value={
                    splitTimeDisplayValues[originalIndex] ??
                    (st.splitTime > 0 ? formatSecondsToDisplay(st.splitTime) : "")
                  }
                  onChangeText={(text) => {
      ...
      >
        <Pressable style={styles.dropdownOverlay} onPress={() => setShowCompetitionPicker(false)}>
          <View
            style={[
                ...
                return (
                  ...
                  >
                    <View style={styles.dropdownOptionContent}>
                      ...
                      {comp.date && <Text style={styles.dropdownOptionSubText}>{comp.date}</Text>}
                    </View>
