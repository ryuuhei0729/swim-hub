...
describe("localizeAuthError", () => {
  ...
  describe("キャンセル処理", () => {
    ...
    it('"キャンセル"を含むメッセージはキャンセルメッセージを返す', () => {
      expect(localizeAuthError("ユーザーがキャンセルしました")).toBe("認証がキャンセルされました");
  ...
  describe("errorMessageMapのカバレッジ", () => {
    ...
    it("トークン関連エラーが正しくマッピングされる", () => {
      ...
      expect(localizeAuthError("token expired")).toBe("認証トークンの有効期限が切れました");
    });
    ...
    it("ネットワークエラーが正しくマッピングされる", () => {
      ...
      expect(localizeAuthError("timeout")).toBe("接続がタイムアウトしました。再度お試しください");
    });

    it("Apple/Google認証エラーが正しくマッピングされる", () => {
      expect(localizeAuthError("apple authentication failed")).toBe("Apple認証に失敗しました");
      expect(localizeAuthError("google sign in failed")).toBe("Googleサインインに失敗しました");
    });
...
describe("localizeSupabaseAuthError", () => {
  describe("null/undefined入力", () => {
    it("nullはジェネリックメッセージを返す（localizeAuthErrorに空文字列で委譲）", () => {
      expect(localizeSupabaseAuthError(null)).toBe("認証エラーが発生しました。再度お試しください");
    });
  ...
  describe("フィールド抽出優先度", () => {
    it("messageフィールドのみのオブジェクトはmessageを使用する", () => {
      expect(localizeSupabaseAuthError({ message: "invalid login credentials" })).toBe(
        "メールアドレスまたはパスワードが正しくありません",
      );
    });

    it("error_descriptionフィールドのみのオブジェクトはerror_descriptionを使用する", () => {
      expect(localizeSupabaseAuthError({ error_description: "token expired" })).toBe(
        "認証トークンの有効期限が切れました",
      );
    });

    it("errorフィールドのみのオブジェクトはerrorを使用する", () => {
      expect(localizeSupabaseAuthError({ error: "access_denied" })).toBe(
        "アクセスが拒否されました",
      );
    });
  });
  ...
  describe("全フィールドがundefinedの場合のフォールバック", () => {
    it("空オブジェクトはジェネリックメッセージを返す", () => {
      expect(localizeSupabaseAuthError({})).toBe("認証エラーが発生しました。再度お試しください");
    });
