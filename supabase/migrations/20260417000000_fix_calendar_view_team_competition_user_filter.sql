-- calendar_viewのチーム大会セクションにuser_idフィルターを追加
-- チーム練習ログでは修正済みだったが、チーム大会の records/entries EXISTS チェックに
-- user_id フィルターが欠落していた。
-- 他メンバーの記録があるだけで 'record' 型と判定され、
-- フロントが自分の記録を取得すると 0 件になる問題を修正。
...
-- 修正: records/entries の EXISTS に user_id フィルターを追加
SELECT "c"."id",
  ...
  WHERE (("c"."team_id" IS NOT NULL) AND (EXISTS ( SELECT 1
          ...
          WHERE (("r"."competition_id" = "c"."id") AND ("r"."user_id" = (SELECT "auth"."uid"())))))) AND (NOT (EXISTS ( SELECT 1
          FROM "public"."entries" "e"
          WHERE (("e"."competition_id" = "c"."id") AND ("e"."user_id" = (SELECT "auth"."uid"())))))))
UNION ALL
...
UNION ALL
-- チーム記録 - 複数日展開対応
-- 修正: records の EXISTS に user_id フィルターを追加
SELECT "c"."id",
    'record'::"text" AS "item_type",
    "d"."date"::date AS "item_date",
    COALESCE("c"."title", '大会'::"text") AS "title",
