-- =============================================================================
-- W-1: replace_practice_logs に挿入したログID一覧 (log_ids) を返す
-- =============================================================================
-- 背景:
--   Web の代理動画アップロード経路は RPC の戻り値 `log_ids`
--   (insert したログIDの配列) に依存して「どのログにどの動画を紐付けるか」を
--   index 突合で解決する。しかし従来の replace_practice_logs は
--   {success, message} しか返さず log_ids を返さなかったため、Web の動画経路が
--   常に偽 (dead-path) になり、コーチが代理入力時に添付した動画が無音破棄されていた。
--
-- 対策:
--   20260618000000 (認可ガード入りの最新定義) を全文ベースにコピーし、差分は最小:
--     1. DECLARE に v_log_ids uuid[] を追加。
--     2. insert ループの RETURNING id INTO v_practice_log_id 直後に
--        array_append で v_log_ids に蓄積。
--     3. 成功レスポンスに log_ids を追加 (既存 success / message は維持: 後方互換)。
--        エラー系レスポンスは不変 (log_ids を付けない)。
--
-- 順序保証 (Web の index 突合の前提):
--   to_jsonb(v_log_ids) の順
--     = array_append の順
--     = insert ループ (SELECT * FROM jsonb_array_elements(p_logs_data)) の順
--     = p_logs_data の順
--   Web 側は logsData を `for menu { for member of members.filter(targetUserIds) }`
--   の順で構築し p_logs_data としてそのまま渡すため、返却される log_ids[i] は
--   p_logs_data[i] に 1:1 対応する。
--
-- 注意:
--   - 認可ガード / 事前 user_id 検証ループ / GRANT・REVOKE (anon 剥奪) /
--     SECURITY DEFINER / OWNER / COMMENT は 20260618000000 から完全保持 (不変)。
--   - 既存 migration (20260618000000) は改変しない。
--   - supabase-schema.ts の Returns は Json のままで型変更不要。
-- =============================================================================
